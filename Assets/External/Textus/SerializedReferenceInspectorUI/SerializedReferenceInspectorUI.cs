...
// Has been made by unity forum member TextusGames
// In hope that Unity team adapt this!
...
// This should be a core unity functions
public static class SerializedReferenceInspectorUI
{
    ...
    public static void ShowContextMenuForManagedReference(this SerializedProperty property)
    {
        ...
        void FillContextMenu()
        {
            ...
            if (!realPropertyType.IsAbstract)
                AddContextMenu(realPropertyType);
            var types = TypeCache.GetTypesDerivedFrom(realPropertyType);
            
            foreach (var currentType in types.Where(currentType => !currentType.IsAbstract))
            {

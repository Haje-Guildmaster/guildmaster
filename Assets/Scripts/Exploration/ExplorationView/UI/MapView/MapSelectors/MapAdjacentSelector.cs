...
namespace GuildMaster.Exploration
{
    ...
    public class MapAdjacentSelector
    {
        ...
        /// <param name="startingNode"> 거리 기준(startingNode로부터 1칸 거리의 노드를 선택함) </param>
        ...
        public async Task<Graph<ExplorationMap.NodeContent>.Node> Select(MapSelectView mapSelectView,
            Graph<ExplorationMap.NodeContent>.Node startingNode, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ret = await mapSelectView.Select(node =>
                    startingNode.Connected.Exists(ind => graph.GetNode(ind) == node), cancellationToken);
            }
            finally
            {
